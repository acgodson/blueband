...
export class LocalDocument {
  ...
  public async loadMetadata(): Promise<Record<string, MetadataTypes>> {
    if (this._metadata == undefined) {
      ...
      try {
        json = "";
        // json = (
        //   await fs.readFile(path.join(this.folderPath, `${this.id}.json`))
        // ).toString();
      } catch (err: unknown) {
        throw new Error(
  ...
  public async loadText(): Promise<string> {
    if (this._text == undefined) {
        try {
             // switch to ipfs after this test
            this._text = (await fs.readFile(path.join("blueband/", `${this.id}.txt`))).toString();
        } catch (err: unknown) {
